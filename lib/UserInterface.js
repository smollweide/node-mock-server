 ...
 * @param {Object} options - the node-mock-server options
 */
...
UserInterface.prototype = extend(UserInterface.prototype, {
	...
	/**
	 * @param {Object} options - the node-mock-server options object
	 ...
	 * @returns {Array} list of methods eg. GET, POST ...
	 * @private
	 */
	_readApiMethods: function (path) {
		...
		this.for(files, function (dirItem) {
			...
			var availableMockResponsesByNameAndType = {};
			...
			this.for(availableMockResponses, function (mockItem) {
				var nameMockItem = mockItem.file.substring(0, mockItem.file.lastIndexOf('.'));
				...
				mockItem.name = nameMockItem;

				if (availableMockResponsesByNameAndType[nameMockItem] === undefined) {
					availableMockResponsesByNameAndType[nameMockItem] = {};
				}

				var mimeType = mime.lookup(mockItem.file);
				if (mimeType !== undefined) {
					availableMockResponsesByNameAndType[nameMockItem][mimeType] = mockItem;
				}
			});

			// prefer json over other file types
			this.forIn(availableMockResponsesByNameAndType, function (key, responsesByType) {
				if (responsesByType) {
					if (responsesByType['application/json'] !== undefined) {
						availableMockResponsesOut.push(responsesByType['application/json']);
					} else {
						availableMockResponsesOut.push(responsesByType[Object.keys(responsesByType)[0]]);
					}
				}
			});

			availableMockResponsesOut.push({
				file: 'middleware',
				name: 'middleware',
			});

			// validation only for availableMockResponsesOut json files
			this.for(availableMockResponsesOut, function (mockItem) {
				var methodStore = this.getMethodStore(itemPath);
				var validationResult;
				try {
					validationResult = methodStore.validation[mockItem.name].counter;
				} catch (err) {
					validationResult = undefined;
				}

				mockItem.isSelected = (selected === mockItem.name);
