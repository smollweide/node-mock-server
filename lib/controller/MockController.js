...
MockController.prototype = extend(MockController.prototype, {
	...
	_handleMockRequest: function (req, res) {
		...
		// Fallback to expectedResponse.json, then to success.json, then fail
		if (!this.existFile(options.responseFilePath)) {
			var responseJson = options.dir + 'mock/' + options.expectedResponse + '.json';
			if (this.existFile(responseJson)) {
				options.responseFilePath = responseJson;
			} else if (this.existFile(options.dir + 'mock/success.json')) {
				...
				throw new Error('no such file: ' + options.responseFilePath);
			}
		}
		...
		if (this.existFile(options.responseHeadersFilePath)) {
			extend(responseHeaders, JSON.parse(this.readFile(options.responseHeadersFilePath)));
		}

		// Add Content-Type header
		var contentType = mime.lookup(options.responseFilePath, this.options.contentType);
	...
	getResponseOptions: function (req, res) {
		...
		var timeout = 0;
		var responseFilePath = this._getResponseFilePath(req, dir, expectedResponse);
    ...
    /**
	 * Gets response file path depending on accepted mime type.
     * @param {Object} req http request
     * @param {string} dir directory path where responses are located
     * @param {string} expectedResponse name of expected response
     * @returns {string} response file path to accepted file or to json file
     * @private
     */
	_getResponseFilePath: function (req, dir, expectedResponse) {
		var acceptTypes = req.header('Accept') || this.options.contentType;
		// TODO: look at available files to decide best match
		var accept = acceptTypes.split(',')[0];
		var fileExtension = '.' + (mime.extension(accept) || 'json');
		return dir + 'mock/' + expectedResponse + fileExtension;
	},
	...
	_findFolder: function (path, options) {
		...
		return output || pathRoot + '/' + pathFolder;
	},
